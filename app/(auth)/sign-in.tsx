...
export default function SignInScreen() {
  ...
  const handleVerify = async (code: string) => {
    ...
    if (signIn.status !== "complete") {
      return { success: false as const, error: "Additional verification is required." };
    }
  ...
  const handleSocial = async (strategy: Parameters<typeof signInWithStrategy>[0]) => {
    setError("");
    posthog.capture('social_auth_attempted', { strategy, screen: 'sign_in' });
      ...
      >
        ...
        >
          <View className="flex-1 px-6 pb-6">
            ...
            <View className="mt-6">
              <GradientButton title="Log In" onPress={handleContinue} loading={submitting} />
            </View>
            ...
            <View className="mt-8 flex-1 items-center justify-end pb-2">
              <Text className="font-poppins-regular text-body-md text-text-secondary">
                ...
                <Link href="/sign-up" className="font-poppins-semibold text-lingua-purple">
                  Sign up
                </Link>
              </Text>
            </View>
